<script setup>
    ...
    const joinGame = (gameMultiplayer)  => {
        storeLobby.joinGame(gameMultiplayer)
    }
    ...
    const mylobby = (myGame) => {
        for (let i = 1; i < myGame.players.length; i++) {
            if (myGame.players[i].user.id === storeAuth.user.id) {
...
<template>
    <div class="divide-y divide-solid divide-gray-200">
        <div v-for="(game, index) in storeLobby.games" :key="game.id" class="flex ps-2 pe-1">
            <div class="flex flex-col grow">
                <div class="text-base pe-4 grow leading-7 flex space-x-2">
                    <span class="pl-1">{{ game.players[0].user.nickname }}</span>
                </div>
                <div class="text-base pe-4 grow leading-10 flex space-x-2">
                    <span class="pl-1">Board: {{ game.board.board_cols }}x{{ game.board.board_rows }}</span>
                </div>
            ...
            <div class="py-1 flex items-center min-w-[1.9rem]">
                ...
                <button v-show="storeLobby.canJoinGame(game)" type="button"
                    class="rounded bg-cyan-500 p-2 m-0.5 text-white" @click="joinGame(game)">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3"
                        stroke="currentColor" class="size-4">
                        <path stroke-linecap="round" stroke-linejoin="round"
